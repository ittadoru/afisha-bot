...
export function MiniApp({ profile, csrfToken, onProfileUpdate, onLogout }: { profile: AccountProfile; csrfToken: string; onProfileUpdate: (profile: AccountProfile) => void; onLogout: () => Promise<void> }) {
  ...
  return (
    <main className={`mini-app${choosingCity ? " city-chooser-active" : ""}${!choosingCity && section === "events" && eventsMode === "map" && !selectedEventId ? " map-glass-active" : ""}${!choosingCity && section === "events" && eventsMode === "list" && !selectedEventId ? " list-material-active" : ""}`}>
