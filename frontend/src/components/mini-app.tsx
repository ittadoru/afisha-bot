...
export function MiniApp({ profile, csrfToken, onProfileUpdate, onLogout }: { profile: AccountProfile; csrfToken: string; onProfileUpdate: (profile: AccountProfile) => void; onLogout: () => Promise<void> }) {
  ...
  if (caseMatch) {
    return <CaseDetailScreen casePublicId={caseMatch[1]} csrfToken={csrfToken} onBack={() => {
      const returnTo = (location.state as { returnTo?: string } | null)?.returnTo;
      navigate(returnTo ?? "/app/cases");
    }} />;
  }
  ...
  const canvas = choosingCity
    ? "discovery"
    : selectedEventId || selectedLookingPostId || section === "create"
      ? "event"
...
function HomeTopBar({ profile, eventsMode, showViewSwitch, unread, onModeChange, onProfile, onNotifications }: { profile: AccountProfile; eventsMode: EventsMode; showViewSwitch: boolean; unread: number; onModeChange: (mode: EventsMode) => void; onProfile: () => void; onNotifications: () => void }) {
  return <header className="home-top-bar" aria-label="Навигация главного экрана">
    <button className="floating-round-control avatar-control" data-material="chrome" type="button" aria-label="Открыть профиль" onClick={onProfile}>
      <UserAvatar
    ...
    {showViewSwitch ? <div className="floating-segment view-segment" data-material="chrome" role="group" aria-label="Вид событий">
    ...
    </div> : <span className="home-mode-title" data-material="chrome"><UserRoundSearch aria-hidden="true" />Компания</span>}
    <button className="floating-round-control notification-control" data-material="chrome" type="button" aria-label={`Открыть уведомления${unread ? `. Непрочитанных: ${unread}` : ""}`} onClick={onNotifications}>
...
function SubpageHeader({ title, onBack, action }: { title: string; onBack: () => void; action?: ReactNode }) {
  return <header className="subpage-header" data-material="chrome"><button type="button" aria-label="Вернуться на главный экран" onClick={onBack}><ArrowLeft aria-hidden="true" /></button><strong>{title}</strong>{action ?? <span aria-hidden="true" />}</header>;
}
...
function HomeModeDock({ active, cityName, onChooseCity, onSelect, onCreate }: { active: "events" | "people"; cityName: string | null; onChooseCity: () => void; onSelect: (section: "events" | "people") => void; onCreate: () => void }) {
  return <div className="home-bottom-controls">
    <button className="dock-city-control" data-material="chrome" type="button" aria-label={`Выбрать город${cityName ? `. Сейчас: ${cityName}` : ""}`} onClick={onChooseCity}><MapPin aria-hidden="true" /></button>
    <div className="floating-segment mode-segment" data-material="chrome" role="group" aria-label="Разделы главного экрана">
    ...
    <button className="create-fab" data-material="critical" type="button" aria-label={active === "events" ? "Создать событие" : "Создать объявление"} onClick={onCreate}><Plus aria-hidden="true" /></button>
  </div>;
}
...
function EventPage({ eventId, csrfToken, onClose, onOpenChat }: { eventId: string; csrfToken: string; onClose: () => void; onOpenChat: () => void }) {
  ...
  const openOrganizer = () => {
    ...
    navigate(`/app/profile/${event.organizer_public_id}`, { state: { returnTo: `/app/event/${eventId}` } });
  };
  const share = async () => {
  ...
  return <section className={`event-detail-page${event ? ` category-${event.category_slug}` : ""}`}>
    ...
    {failed ? <DemoState state="error" onClose={onClose} /> : !event ? <LoadingScreen variant="section" /> : <><div className="event-detail-scroll"><EventPhoto className="event-detail-hero" src={event.photo_url} cardSrc={event.photo_card_url} alt={`Фотография события «${event.title}»`} priority /><main className="event-detail-content">
      ...
      {event.kind === "regular" ? <button className="organizer-link" data-material="content" type="button" onClick={openOrganizer}><span className="demo-avatar">{event.organizer_name?.[0] ?? "?"}</span><span className="organizer-copy"><small>Организатор</small><strong>{event.organizer_name}</strong><span>{event.organizer_status === "trusted" ? "Доверенный организатор" : "Новый организатор"}</span></span><ChevronRight aria-hidden="true" /></button> : <div className="municipal-organizer" data-material="content"><Sparkles aria-hidden="true" /><span><small>Организатор отсутствует</small><strong>Общественное событие</strong></span></div>}
    ...
    {showPrimaryAction && <footer className={`event-sticky-cta${event.viewer_is_organizer ? " solo" : ""}`} data-material="chrome">{event.viewer_is_organizer ? <Button onClick={() => setManaging(true)}>Управлять событием</Button> : event.viewer_membership === "none" ? <Button disabled={busy} onClick={() => void join()}>{event.capacity !== null && event.available_places === 0 ? "Встать в очередь" : "Присоединиться"}</Button> : event.viewer_membership === "participating" ? <Button variant="outline" disabled={busy} onClick={() => void leave()}>Отказаться от участия</Button> : event.viewer_membership === "waitlisted" ? <Button variant="outline" disabled={busy} onClick={() => void leave()}>Покинуть очередь</Button> : null}{!event.viewer_is_organizer && event.event_scope !== "community" && event.kind !== "special" && <button className="event-report-button" data-material="micro" type="button" aria-label="Пожаловаться на событие" onClick={() => navigate(`/app/report/event/${event.id}`, { state: { returnTo: `/app/event/${event.id}` } })}><Flag aria-hidden="true" /></button>}</footer>}</>}
  </section>;
}
...
function CaseDetailScreen({ casePublicId, csrfToken, onBack }: { casePublicId: string; csrfToken: string; onBack: () => void }) {
  ...
  return <main className="mini-app" data-canvas="safety"><SubpageHeader title={casePublicId} onBack={onBack} />{error ? <CompactListState title="Обращение недоступно" action="Назад" onAction={onBack} /> : !detail ? <LoadingScreen variant="section" /> : <section className="feed case-detail"><h1>{detail.status === "resolved" ? "Обращение решено" : "Обращение рассматривается"}</h1><ol className="status-timeline">{detail.timeline.map((entry) => <li key={`${entry.event_type}-${entry.created_at}`}><span aria-hidden="true" /><div><strong>{entry.public_label}</strong><small>{new Date(entry.created_at).toLocaleString("ru-RU")}</small></div></li>)}</ol><p className={`case-appeal-step ${detail.can_appeal ? "available" : ""}`}>{appealStep(detail)}</p>{detail.can_appeal ? <div className="profile-editor"><label>Апелляция<textarea maxLength={500} value={appeal} onChange={(event) => setAppeal(event.target.value)} /></label>{appealError && <p className="form-error" role="alert">{appealError}</p>}<Button disabled={busy || !appeal.trim()} onClick={() => void submitAppeal()}>{busy ? "Отправляем…" : "Отправить апелляцию"}</Button></div> : detail.appeal ? <section className="case-appeal-readonly" data-material="text-heavy"><h2>Апелляция</h2><p>{detail.appeal.explanation}</p><small>Отправлена {new Date(detail.appeal.created_at).toLocaleString("ru-RU")}</small></section> : null}</section>}</main>;
}
