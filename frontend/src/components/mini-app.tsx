...
function LookingPostSheet({ postId, csrfToken, onClose }: { postId: string; csrfToken: string; onClose: () => void }) {
  ...
  return <div className="event-sheet-backdrop" onClick={onClose}><section className="event-sheet street-group-sheet" onClick={(event) => event.stopPropagation()}><button className="sheet-handle" type="button" aria-label="Закрыть" onClick={onClose} />{post ? <><div className="sheet-actions"><button type="button" onClick={() => void share()} aria-label="Поделиться идеей"><Share2 /> Поделиться</button>{post.is_author && post.status === "active" && <button type="button" className="danger-action" disabled={busy} onClick={() => void withdraw()}>Снять идею</button>}</div><span className="category-chip">{post.category}</span><h2>{post.title}</h2><p>{post.body}</p><p className="idea-status">● {statusText}</p><h3>Вопросы и ответы</h3>{questions.map((item) => <article className="profile-event" key={item.id}><strong>{item.question}</strong><span>{item.answer}</span></article>)}{pending.length > 0 && <><h3>{post.is_author ? "Новые вопросы" : "Ваш вопрос ожидает ответа"}</h3>{pending.map((item) => <article className="profile-event" key={item.id}>{post.is_author && item.asker && <small>{item.asker.display_name}</small>}<strong>{item.question}</strong>{post.is_author && post.status === "active" && <><textarea value={answers[item.id] ?? ""} onChange={(event) => setAnswers({ ...answers, [item.id]: event.target.value })} maxLength={300} placeholder="Напишите ответ" /><Button disabled={busy || !answers[item.id]?.trim()} onClick={() => void answer(item.id)}>Ответить</Button></>}</article>)}</>}{!post.is_author && post.status === "active" && <div className="profile-editor"><textarea value={question} onChange={(event) => setQuestion(event.target.value)} maxLength={200} placeholder="Задайте вопрос автору" /><Button disabled={busy || !question.trim()} onClick={() => void ask()}>Спросить</Button></div>}{message && <p className="success-message">{message}</p>}</> : <DemoState state="loading" />}</section></div>;
}
