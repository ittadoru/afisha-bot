...
function Profile({
  ...
  return <section className="feed profile-screen"><div className="profile-card">{profile.avatar_url ? <img className="profile-avatar profile-photo" src={profile.avatar_url} alt="Ваш аватар" /> : <span className="profile-avatar" style={{ backgroundColor: color }}>{profile.display_name[0]}</span>}<div><p className="section-kicker">Ваш профиль</p><h1>{profile.display_name}</h1><p>ID <button className="copy-id" type="button" aria-label={`Скопировать ID ${profile.public_id}`} onClick={() => void navigator.clipboard.writeText(profile.public_id)}>{profile.public_id}</button> · {profile.organizer_status === "trusted" ? "Доверенный организатор" : "Новый организатор"}</p></div></div><div className="profile-stats"><div><strong>{profile.upcoming_count ?? 0}</strong><span>будущих</span></div><div><strong>{profile.completed_count ?? 0}</strong><span>завершено</span></div><div><strong>{profile.successful_events ?? 0}</strong><span>успешных</span></div></div><p className="profile-bio">{profile.bio || "Описание пока не заполнено"}</p><div className="profile-primary-actions"><Button onClick={() => setEditing(true)}>Редактировать профиль</Button><label className="file-picker compact-picker">{avatarBusy ? <span>Загружаем фото…</span> : <span>{profile.avatar_url ? "Заменить фотографию" : "Добавить фотографию"}</span>}<input type="file" accept="image/jpeg,image/png,image/webp" disabled={avatarBusy} onChange={(event) => { const file = event.target.files?.[0]; if (file) void upload(file); }} /></label></div>{profile.avatar_url && <button className="profile-remove-photo" type="button" disabled={avatarBusy} onClick={() => void removeAvatar()}>Удалить фотографию</button>}{message && <p className="success-message" role="status">{message}</p>}<h2 className="group-title">События</h2><button className="menu-row" type="button" onClick={() => setEventMode("upcoming")}><span>Будущие события</span><ChevronRight /></button><button className="menu-row" type="button" onClick={() => setEventMode("completed")}><span>Завершённые события</span><ChevronRight /></button><h2 className="group-title">Открыть профиль</h2><div className="profile-lookup"><input aria-label="Восьмизначный ID профиля" inputMode="numeric" maxLength={8} placeholder="Восьмизначный номер" value={lookup} onChange={(event) => setLookup(event.target.value.replace(/\D/g, ""))} /><Button disabled={lookup.length !== 8} onClick={() => void openPublic()}>Открыть</Button></div><Button variant="outline" onClick={() => void onLogout()}>Выйти</Button>{import.meta.env.DEV && <><h2 className="group-title">Предпросмотр состояний</h2><div className="state-buttons"><Button variant="outline" onClick={() => onPreview("loading")}>Загрузка</Button><Button variant="outline" onClick={() => onPreview("empty")}>Пусто</Button><Button variant="outline" onClick={() => onPreview("error")}>Ошибка</Button></div></>}</section>;
}
...
function DemoState({ state, onClose }: { state: Exclude<PreviewState, null>; onClose?: () => void }) {
  ...
  if (state === "loading") return <section className="skeleton-screen" role="status" aria-label="Загружаем"><div className="skeleton-hero" /><div className="skeleton-lines"><span /><span /><span /></div><div className="skeleton-card" /><span className="sr-only">Загружаем содержимое</span></section>;
