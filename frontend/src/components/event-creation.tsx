...
export function EventCreation({ city, categories, csrfToken, organizerStatus, onDirtyChange, registerDiscard, onCancel, onFinished }: EventCreationProps) {
  ...
  const [publicAddressConfirmed, setPublicAddressConfirmed] = useState(false);
  ...
  const dirty = Boolean(title || description || categoryId || startsAt || endsAt || location || locationNote || photo || limited);
  ...
  useEffect(() => {
    setPublicAddressConfirmed(false);
  }, [location, visibility]);
  ...
  const validateStep = (current: number): string | null => {
    ...
    if (current === 3) {
      ...
      if (!location.street && visibility !== "exact_public") return "Без определённой улицы доступен только публичный точный адрес.";
    }
    if (current === 4 && !photo) return "Добавьте фотографию события.";
    return null;
  };
  ...
  const submit = async () => {
    ...
    if (visibility === "exact_public" && !publicAddressConfirmed) {
    ...
    const payload = JSON.stringify({
      ...
      address_visibility: visibility, exact_address_confirmed: publicAddressConfirmed,
  ...
  return <section className="event-wizard"><header className="event-wizard-header"><button className="text-back" type="button" onClick={() => void requestCancel()}>← Закрыть</button><span>Шаг {step} из 4</span></header><div className="wizard-progress" aria-label={`Шаг ${step} из 4`}>{[1, 2, 3, 4].map((number) => <button type="button" key={number} className={number <= step ? "active" : ""} disabled={number >= step} onClick={() => setStep(number)}>{number}</button>)}</div>{step === 1 && <section className="wizard-step"><p className="section-kicker">Основное</p><h1>Расскажите о событии</h1><label>Название<input value={title} maxLength={60} onChange={(event) => change(setTitle, event.target.value)} placeholder="Например, прогулка у моря" /><small>{title.length}/60</small></label><label>Категория<select value={categoryId} onChange={(event) => change(setCategoryId, event.target.value)}><option value="" disabled>Выберите категорию</option>{selectableCategories.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label>Описание<textarea value={description} maxLength={1000} onChange={(event) => change(setDescription, event.target.value)} placeholder="Что будет происходить и что важно знать участникам" /><small>{description.length}/1000</small></label></section>}{step === 2 && <section className="wizard-step"><p className="section-kicker">Время и участие</p><h1>Когда встречаемся?</h1><label>Начало<input type="datetime-local" value={startsAt} min={minimumLocalDate(organizerStatus === "trusted" ? 1 : 6)} onChange={(event) => change(setStartsAt, event.target.value)} /></label><label>Окончание<input type="datetime-local" value={endsAt} min={startsAt} onChange={(event) => change(setEndsAt, event.target.value)} /></label><label className="capacity-toggle"><input type="checkbox" checked={limited} onChange={(event) => change(setLimited, event.target.checked)} /><span><strong>Ограничить количество мест</strong><small>Организатор не входит в лимит.</small></span></label>{limited && <label>Количество участников<input type="number" min="3" inputMode="numeric" value={capacity} onChange={(event) => change(setCapacity, event.target.value)} /></label>}</section>}{step === 3 && <section className="wizard-step"><p className="section-kicker">Место</p><h1>Где пройдёт событие?</h1><div className="selected-city static-city"><MapPin /><span><small>Город</small>{city.name}</span></div><EventMap embedded selecting city={city} onLocationChange={(value) => { change(setLocation, value); setLocationNote(""); }} />{location && !location.house_number && <p className="state-hint">Точка на карте сохранится точно, но OpenStreetMap знает этот адрес только до улицы. При необходимости добавьте дом или ориентир.</p>}{location && <label>Уточнение места (необязательно)<input value={locationNote} maxLength={80} onChange={(event) => change(setLocationNote, event.target.value)} placeholder="Например: дом 12, у главного входа" /><small>{locationNote.length}/80</small></label>}<fieldset className="visibility-options"><legend>Кто увидит точный адрес</legend><VisibilityOption value="exact_public" selected={visibility} onSelect={(value) => change(setVisibility, value)} title="Все" text="Точное место сразу видно в карточке." /><VisibilityOption value="exact_participants" selected={visibility} onSelect={(value) => change(setVisibility, value)} disabled={Boolean(location && !location.street)} title="Только участники" text="Остальные увидят только улицу." /><VisibilityOption value="street_only" selected={visibility} onSelect={(value) => change(setVisibility, value)} disabled={Boolean(location && !location.street)} title="Только улица" text="Точное место видно только организатору." /></fieldset></section>}{step === 4 && <section className="wizard-step"><p className="section-kicker">Фото и отправка</p><h1>Проверьте событие</h1><EventPhotoUploader csrfToken={csrfToken} value={photo} onChange={(value) => change(setPhoto, value)} />{photo && <article className="event-preview"><img src={photo.preview_url} alt="Фотография события" /><div><span className="category-chip">{category?.name}</span><h2>{title}</h2><p>{formatMoscow(startsAt)} — {formatMoscow(endsAt)}</p><p><MapPin /> {location?.display_name}{locationNote ? ` · ${locationNote}` : ""}</p><p><Users /> {limited ? `До ${capacity} участников` : "Без ограничения мест"}</p><p>{description}</p></div></article>}{visibility === "exact_public" && photo && <label className="address-confirmation"><input type="checkbox" checked={publicAddressConfirmed} onChange={(event) => setPublicAddressConfirmed(event.target.checked)} /><span><strong>Я понимаю, что точный адрес увидят все</strong><small>Уже увиденный адрес невозможно сделать неизвестным.</small></span></label>}</section>}{error && <p className="form-error wizard-error" role="alert">{error}</p>}<footer className="wizard-actions">{step > 1 && <Button variant="outline" disabled={busy} onClick={() => { setError(""); setStep((current) => current - 1); }}>Назад</Button>}{step < 4 ? <Button onClick={goNext}>Продолжить</Button> : <Button disabled={busy || !photo || (visibility === "exact_public" && !publicAddressConfirmed)} onClick={() => void submit()}>{busy ? "Отправляем…" : organizerStatus === "trusted" ? "Опубликовать" : "Отправить на проверку"}</Button>}</footer></section>;
}
...
function eventErrorMessage(reason: unknown): string {
  ...
  const messages: Record<string, string> = {
    ...
    point_outside_city: "Выбранная точка находится за границей города.",
