...
function LandingPage() {
  return (
    <main className="site-shell" data-canvas="public">
      <header className="site-header" data-material="chrome">
...
function PublicEventPage({ eventId }: { eventId: string }) {
  ...
  if (failed) return <main className="public-event-page" data-canvas="public"><section className="public-event-card" data-material="text-heavy"><h1>Событие недоступно</h1><p>Возможно, оно ещё не опубликовано или было скрыто.</p><Button asChild><a href="/">На главную</a></Button></section></main>;
  if (!event) return <main className="public-event-page" data-canvas="public"><LoadingScreen /></main>;
  return <main className="public-event-page" data-canvas="public"><header className="site-header" data-material="chrome"><a className="brand" href="/">Афиша</a><Button asChild><a href={`/app/event/${event.id}`}>Открыть Mini App</a></Button></header><article className={`public-event-card${event.kind === "special" ? " special-event" : ""}`} data-material="text-heavy"><img src={event.photo_url} srcSet={event.photo_card_url ? `${event.photo_card_url} 640w, ${event.photo_url} 1200w` : undefined} sizes="(max-width: 720px) calc(100vw - 32px), 640px" width="1200" height="900" decoding="async" fetchPriority="high" alt={`Фотография события «${event.title}»`} /><div><span className="category-chip" data-material="micro">{event.kind === "special" ? "Особое · Общественное событие" : event.category}</span><h1>{event.title}</h1><p><CalendarDays /> {new Date(event.starts_at).toLocaleString("ru-RU", { timeZone: "Europe/Moscow" })}</p><p><Map /> {event.visible_address}</p>{event.lifecycle_status === "cancelled" && <p className="form-error">Событие отменено</p>}<p className="public-event-description">{event.description}</p>{event.kind === "regular" && <p><strong>Организатор:</strong> {event.organizer_name} · {event.organizer_status === "trusted" ? "доверенный" : "новый"}</p>}<Button asChild><a href={`/app/event/${event.id}`}>Открыть в приложении</a></Button></div></article></main>;
}
