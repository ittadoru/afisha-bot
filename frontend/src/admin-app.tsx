...
function componentTargetLabel(subject: string, component: string) { return `${componentLabel(component).toLocaleLowerCase("ru-RU")} · ${subjectLabel(subject).toLocaleLowerCase("ru-RU")}`; }
function actionButtonLabel(action: CaseAction, detail: ModerationCaseDetail) { const component = componentLabel(detail.target.component).toLocaleLowerCase("ru-RU"); return ({ dismiss: "Отклонить жалобу", hide_component: `Скрыть ${component}`, hold_for_correction: `Снять до исправления: ${component}`, hide_subject: `Скрыть ${subjectLabel(detail.target.subject_type).toLocaleLowerCase("ru-RU")} целиком`, upheld: "Оставить решение в силе", reversed: "Отменить решение" } as Record<CaseAction, string>)[action]; }
