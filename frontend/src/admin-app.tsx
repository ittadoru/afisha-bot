...
function actionButtonLabel(action: CaseAction, detail: ModerationCaseDetail) { const shortComponent: Record<string, string> = { photo: "фотографию", avatar: "аватар", background: "фон", display_name: "имя", bio: "описание", title: "название", body: "описание", answer: "ответ", message: "сообщение" }; if (action === "hide_subject" && detail.target.subject_type === "event") return "Скрыть событие"; return ({ dismiss: "Отклонить", hide_component: `Скрыть ${shortComponent[detail.target.component] ?? "компонент"}`, hold_for_correction: "Снять до исправления", hide_subject: "Скрыть полностью", upheld: "Оставить в силе", reversed: "Отменить решение" } as Record<CaseAction, string>)[action]; }
function actionConsequence(action: CaseAction, detail: ModerationCaseDetail) { if (action === "hide_component") return "Выбранный компонент будет скрыт. Решение можно будет обжаловать."; if (action === "hold_for_correction") return "Объект будет скрыт до исправления и повторной проверки."; if (action === "hide_subject" && detail.target.subject_type === "event") return "Событие станет недоступно сразу и будет окончательно очищено после окна апелляции."; if (action === "hide_subject") return "Объект будет скрыт целиком. Решение можно будет обжаловать."; return "Решение будет записано в историю модерации."; }
