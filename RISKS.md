...
7. Создать основу базы
пользователи;
входы через Telegram;
профили;
города;
категории;
события;
...
Каждый следующий этап будет дополнять базу, а не создавать всё повторно.
8. Сделать настоящий вход
приложение получает данные пользователя от Telegram;
сервер проверяет их;
при первом входе создаёт пользователя и профиль;
повторный вход открывает существующий профиль;
один Telegram-аккаунт не может создать несколько пользователей.
Результат: каждый из пяти друзей входит под своим профилем.
9. Настроить города, категории и карту
Махачкала;
Хасавюрт;
Дербент;
все 16 категорий;
...
Этап D. Администратор и события
