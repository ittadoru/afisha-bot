...
def normalize_bio(value: str | None) -> str | None:
    ...
    if len(normalized) > 150 or any(ord(character) < 32 and character not in "\n\t" for character in normalized):
        raise ProfileError("invalid_bio")
    return normalized or None


async def session_user_id(
    engine: AsyncEngine, *, token: str, auth_secret: bytes, csrf_token: str | None = None
) -> UUID | None:
...
async def load_profile(engine: AsyncEngine, *, public_id: str | None = None, user_id: UUID | None = None) -> ProfileView:
    condition = "p.public_id = :lookup" if public_id is not None else "p.user_id = :lookup"
    ...
    return ProfileView(
        user_id=row["user_id"], public_id=row["public_id"], display_name=row["display_name"],
        bio=row["bio"], selected_city_id=row["selected_city_id"], city_name=row["city_name"],
        avatar_asset_id=row["avatar_asset_id"], version=row["version"],
        next_name_change_at=next_change, organizer_status=row["organizer_status"],
        successful_events=row["successful_events"], upcoming_count=row["upcoming_count"],
        completed_count=row["completed_count"],
    )


async def update_profile(
    engine: AsyncEngine, *, user_id: UUID, display_name: str, bio: str | None,
    selected_city_id: UUID, expected_version: int
) -> ProfileView:
    ...
    async with engine.begin() as connection:
        row = (
            await connection.execute(
                text("SELECT display_name, display_name_changed_at FROM accounts.profiles WHERE user_id=:id FOR UPDATE"),
                {"id": user_id},
            )
        ).mappings().one_or_none()
        if row is None:
            raise ProfileNotFound
        city_exists = await connection.scalar(text("SELECT 1 FROM discovery.cities WHERE id=:id AND is_active"), {"id": selected_city_id})
        if city_exists is None:
            raise ProfileError("invalid_city")
        name_changed = name != row["display_name"]
        if name_changed and row["display_name_changed_at"] is not None and row["display_name_changed_at"] + NAME_COOLDOWN > datetime.now(UTC):
        ...
        updated = await connection.scalar(
            ...
            {"name": name, "bio": about, "city": selected_city_id, "id": user_id, "version": expected_version},
        )
        if updated is None:
...
async def create_report(engine: AsyncEngine, *, reporter_id: UUID, subject: ProfileView, reason: str, comment: str | None) -> None:
    note = comment.strip() if comment is not None else None
    if note is not None and len(note) > 300:
        raise ProfileError("invalid_report")
    if reason not in {"photo", "display_name", "bio", "other"} or (reason == "other" and not note):
        raise ProfileError("invalid_report")
    ...
    try:
        async with engine.begin() as connection:
            await connection.execute(
                ...
                {"id": uuid4(), "reporter": reporter_id, "subject": subject.user_id, "reason": reason, "comment": note, "avatar": subject.avatar_asset_id},
            )
    except IntegrityError as error:
        raise ProfileConflict("report_already_open") from error


async def profile_events(engine: AsyncEngine, *, user_id: UUID, state: str, limit: int, offset: int) -> list[dict[str, Any]]:
    ...
    async with engine.connect() as connection:
        rows = (
            await connection.execute(
                ...
                {"user": user_id, "status": lifecycle, "limit": limit, "offset": offset},
            )
        ).mappings().all()
    return [dict(row) for row in rows]


async def account_events(engine: AsyncEngine, *, user_id: UUID, state: str, limit: int, offset: int) -> list[dict[str, Any]]:
    lifecycle = "published" if state == "upcoming" else "finished"
