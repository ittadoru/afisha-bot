...
async def create_looking_post(
    ...
    *,
    ...
    title = _compact(title, 30)
    body = _compact(body, 300)
    ...
    async with engine.begin() as connection:
        ...
        if previous is not None:
            ...
            city = await connection.scalar(
                text(
                    "SELECT 1 FROM discovery.cities WHERE id=:id AND is_active AND looking_posts_enabled"
                ),
                {"id": city_id},
            )
            category = await connection.scalar(
                text(
                    "SELECT 1 FROM discovery.categories WHERE id=:id AND is_active AND organizer_selectable AND NOT is_special"
                ),
                {"id": category_id},
            )
            if city is None:
                raise LookingPostError("city_not_available")
            if category is None:
                raise LookingPostError("category_not_available")
            ...
            await _remember_request(
                connection, user_id, idempotency_key, "create", fingerprint, post_id
            )
...
async def looking_post_feed(
    ...
    *,
    ...
    if sort == "popular":
        ...
        cursor_sql = (
            "AND (like_count, created_at, id) < (:likes, :at, :id)" if cursor else ""
        )
    elif sort == "old":
        order = "created_at ASC, id ASC"
        cursor_sql = "AND (created_at,id) > (:at,:id)" if cursor else ""
    else:
    ...
    return items, (
        (
            int(last["like_count"]) if sort == "popular" else None,
            last["created_at"],
            last["id"],
        )
        if last
...
def _feed_item(row: Any, viewer_id: UUID | None) -> dict[str, Any]:
    ...
    return {
        ...
        "remaining_seconds": remaining if effective_status == "active" else 0,
        ...
        "viewer_liked": bool(row["viewer_liked"]) if viewer_id else False,
        "is_author": author,
        "author": {
...
async def looking_post_detail(
    engine: AsyncEngine, *, post_id: UUID, viewer_id: UUID | None
) -> dict[str, Any]:
    async with engine.connect() as connection:
        row = (
            (
...
async def set_looking_post_like(
    engine: AsyncEngine, *, post_id: UUID, user_id: UUID, active: bool
) -> dict[str, Any]:
    async with engine.begin() as connection:
        post = (
            (
                await connection.execute(
                    text(
                        "SELECT author_user_id FROM discovery.looking_posts WHERE id=:id AND status='active' AND expires_at>now() FOR UPDATE"
                    ),
                    {"id": post_id},
                )
            )
        ...
        if post is None:
            exists = await connection.scalar(
                text("SELECT 1 FROM discovery.looking_posts WHERE id=:id"),
                {"id": post_id},
            )
            if exists:
        ...
        if post["author_user_id"] == user_id:
            raise LookingPostError("author_cannot_like")
        await connection.execute(
            text("""
        ...
        count = await connection.scalar(
            text(
                "SELECT count(*) FROM discovery.looking_post_likes WHERE looking_post_id=:post AND active"
            ),
            {"post": post_id},
        )
    ...
    async with engine.connect() as connection:
        post = (
            (
                await connection.execute(
                    text(
                        "SELECT author_user_id FROM discovery.looking_posts WHERE id=:id"
                    ),
                    {"id": post_id},
                )
            )
        ...
        if post is None or (
            post["author_user_id"] != viewer_id
            and await connection.scalar(
                text(
                    "SELECT 1 FROM discovery.looking_posts WHERE id=:id AND status='hidden'"
                ),
                {"id": post_id},
            )
        ):
        ...
        own = (
            (
                await connection.execute(
                    text(
                        "SELECT id,question,created_at FROM discovery.looking_post_questions WHERE looking_post_id=:post AND asker_user_id=:user AND answer IS NULL"
                    ),
                    {"post": post_id, "user": viewer_id},
                )
            )
            .mappings()
            .all()
            ...
            pending = [
                {
                    **dict(row),
                    "asker": {
                        "public_id": row["public_id"],
                        "display_name": row["display_name"],
                        "avatar_thumbnail_url": f"/api/profiles/{row['public_id']}/avatar?size=64&v={row['profile_version']}"
                        if row["avatar_asset_id"]
                        else None,
                        "avatar_url": f"/api/profiles/{row['public_id']}/avatar?size=256&v={row['profile_version']}"
                        if row["avatar_asset_id"]
                        else None,
                    },
                }
    ...
    try:
        async with engine.begin() as connection:
            ...
            if post is None:
                raise LookingPostClosed("idea_closed")
            if post["author_user_id"] == user_id:
                raise LookingPostError("author_cannot_ask")
            question_id = uuid4()
            await connection.execute(
                text(
                    "INSERT INTO discovery.looking_post_questions (id,looking_post_id,asker_user_id,question) VALUES (:id,:post,:user,:question)"
                ),
                {
                    "id": question_id,
                    "post": post_id,
            ...
            await _notify(
                connection,
                recipient=post["author_user_id"],
                kind="looking_post.question",
                title="Новый вопрос к идее",
                body="Откройте «Ищу людей», чтобы ответить.",
                subject_id=post_id,
                link=f"/app/looking/{post_id}",
            )
            return question_id
    except IntegrityError as error:
        raise LookingPostConflict("unanswered_question_exists") from error
...
async def answer_question(
    ...
    *,
    ...
    async with engine.begin() as connection:
        ...
        """),
                    {"question": question_id, "post": post_id, "user": user_id},
                )
            )
            .mappings()
            .one_or_none()
        ...
        if row is None:
            raise LookingPostConflict("question_not_answerable")
        await connection.execute(
            text(
                "UPDATE discovery.looking_post_questions SET answer=:answer,answered_at=now() WHERE id=:id"
            ),
            {"answer": answer, "id": question_id},
        )
        await _remember_request(
            connection, user_id, idempotency_key, "answer", fingerprint, question_id
        )
        await _notify(
            connection,
            recipient=row["asker_user_id"],
            kind="looking_post.answer",
            title="На ваш вопрос ответили",
            body="Откройте идею, чтобы прочитать ответ.",
            subject_id=post_id,
            link=f"/app/looking/{post_id}",
        )
...
async def expire_looking_posts(engine: AsyncEngine) -> int:
    async with engine.begin() as connection:
        ...
        if ids:
            await connection.execute(
                text(
                    "UPDATE discovery.looking_post_questions SET delete_after=now()+interval '24 hours' WHERE looking_post_id = ANY(:ids) AND delete_after IS NULL"
                ),
                {"ids": ids},
            )
    return len(ids)
...
async def withdraw_looking_post(
    engine: AsyncEngine, *, post_id: UUID, user_id: UUID
) -> None:
    async with engine.begin() as connection:
        result = await connection.execute(
            text("""
        ...
        await connection.execute(
            text(
                "UPDATE discovery.looking_post_questions SET delete_after=now()+interval '24 hours' WHERE looking_post_id=:id AND delete_after IS NULL"
            ),
            {"id": post_id},
        )
...
def _fingerprint(action: str, *parts: object) -> str:
    return sha256(
        "\x1f".join((action, *(str(part) for part in parts))).encode()
    ).hexdigest()
...
async def _existing_request(
    ...
    await connection.execute(
        ...
        {"key": f"looking-post:{user_id}:{key}"},
    )
    row = (
        (
            await connection.execute(
                text(
                    "SELECT request_fingerprint, resource_id FROM discovery.looking_post_requests WHERE user_id=:user AND idempotency_key=:key FOR UPDATE"
                ),
...
async def _remember_request(
    ...
    await connection.execute(
        text(
            "INSERT INTO discovery.looking_post_requests (user_id,idempotency_key,action,request_fingerprint,resource_id) VALUES (:user,:key,:action,:fingerprint,:resource)"
        ),
        {
            "user": user_id,
            "key": key,
