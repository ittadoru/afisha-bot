...
from afishabot.modules.accounts.application.profiles import (
    ProfileConflict, ProfileError, ProfileNotFound, ProfileView, account_events, create_report,
    load_profile, profile_events, session_user_id, update_profile,
)
from afishabot.modules.media.application.image_processing import AvatarImageProcessor, UnsafeImageError
...
def dependencies(request: Request) -> tuple[Settings, Redis, AsyncEngine]:
    return cast(Settings, request.app.state.settings), cast(Redis, request.app.state.redis_client), cast(AsyncEngine, request.app.state.database_engine)
...
async def current_user(request: Request, token: str | None, csrf: str | None = None) -> UUID:
    if token is None:
        raise HTTPException(status_code=401, detail="session_required")
    settings, _, engine = dependencies(request)
    user_id = await session_user_id(engine, token=token, csrf_token=csrf, auth_secret=auth_secret(settings))
    if user_id is None:
...
def own_response(profile: ProfileView) -> OwnProfileResponse:
    return OwnProfileResponse(
        public_id=profile.public_id, display_name=profile.display_name, bio=profile.bio,
        selected_city_id=str(profile.selected_city_id) if profile.selected_city_id else None,
        city_name=profile.city_name, avatar_url=f"/api/profiles/{profile.public_id}/avatar?v={profile.version}" if profile.avatar_asset_id else None,
        version=profile.version,
        next_name_change_at=profile.next_name_change_at.isoformat() if profile.next_name_change_at else None,
...
async def get_own_profile(request: Request, token: Annotated[str | None, Cookie(alias=SESSION_COOKIE)] = None) -> OwnProfileResponse:
    _, _, engine = dependencies(request)
    return own_response(await load_profile(engine, user_id=await current_user(request, token)))


@router.patch("/account/profile", response_model=OwnProfileResponse)
async def patch_profile(body: UpdateProfileRequest, request: Request, token: Annotated[str | None, Cookie(alias=SESSION_COOKIE)] = None, csrf: Annotated[str | None, Header(alias=CSRF_HEADER)] = None) -> OwnProfileResponse:
    settings, _, engine = dependencies(request)
    validate_origin(request, settings)
    try:
        profile = await update_profile(engine, user_id=await mutation_user(request, token, csrf), display_name=body.display_name, bio=body.bio, selected_city_id=body.selected_city_id, expected_version=body.version)
...
async def public_profile(public_id: str, request: Request, token: Annotated[str | None, Cookie(alias=SESSION_COOKIE)] = None) -> PublicProfileResponse:
    if not (len(public_id) == 8 and public_id.isdigit()):
        raise HTTPException(status_code=404, detail="profile_not_found")
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail="profile_not_found") from error
    return PublicProfileResponse(public_id=profile.public_id, display_name=profile.display_name, bio=profile.bio, avatar_url=f"/api/profiles/{profile.public_id}/avatar?v={profile.version}" if profile.avatar_asset_id else None, organizer_status=profile.organizer_status, successful_events=profile.successful_events)


@router.get("/profiles/{public_id}/events", response_model=EventsResponse)
async def get_profile_events(public_id: str, request: Request, state: Literal["upcoming", "completed"], offset: Annotated[int, Query(ge=0)] = 0, limit: Annotated[int, Query(ge=1, le=20)] = 10, token: Annotated[str | None, Cookie(alias=SESSION_COOKIE)] = None) -> EventsResponse:
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail="profile_not_found") from error
    items = await profile_events(engine, user_id=profile.user_id, state=state, limit=limit + 1, offset=offset)
...
async def get_account_events(request: Request, state: Literal["upcoming", "completed"], offset: Annotated[int, Query(ge=0)] = 0, limit: Annotated[int, Query(ge=1, le=20)] = 10, token: Annotated[str | None, Cookie(alias=SESSION_COOKIE)] = None) -> EventsResponse:
    _, _, engine = dependencies(request)
...
async def report_profile(public_id: str, body: ReportRequest, request: Request, token: Annotated[str | None, Cookie(alias=SESSION_COOKIE)] = None, csrf: Annotated[str | None, Header(alias=CSRF_HEADER)] = None) -> dict[str, str]:
    settings, _, engine = dependencies(request)
    validate_origin(request, settings)
    reporter = await mutation_user(request, token, csrf)
    try:
        await create_report(engine, reporter_id=reporter, subject=await load_profile(engine, public_id=public_id), reason=body.reason, comment=body.comment)
    except ProfileNotFound as error:
        raise HTTPException(status_code=404, detail="profile_not_found") from error
    except ProfileConflict as error:
        raise HTTPException(status_code=409, detail=str(error)) from error
...
async def put_avatar(request: Request, token: Annotated[str | None, Cookie(alias=SESSION_COOKIE)] = None, csrf: Annotated[str | None, Header(alias=CSRF_HEADER)] = None) -> OwnProfileResponse:
    settings, _, engine = dependencies(request)
    validate_origin(request, settings)
    ...
    try:
        ...
        raise HTTPException(status_code=422, detail=str(error)) from error
    checksum = await to_thread.run_sync(lambda: hashlib.sha256(destination.read_bytes()).hexdigest())
    old_path: Path | None = None
    async with engine.begin() as connection:
        old = (await connection.execute(text("""SELECT a.id, a.storage_key FROM accounts.profiles p LEFT JOIN media.assets a ON a.id=p.avatar_asset_id WHERE p.user_id=:user FOR UPDATE OF p"""), {"user": user_id})).mappings().one()
        await connection.execute(text("""INSERT INTO media.assets (id,owner_user_id,purpose,state,storage_key,mime_type,byte_size,width,height,checksum_sha256)
            VALUES (:id,:user,'profile_avatar','ready',:key,'image/webp',:size,256,256,:checksum)"""), {"id": asset_id, "user": user_id, "key": f"avatars/{asset_id}.webp", "size": destination.stat().st_size, "checksum": checksum})
        await connection.execute(text("UPDATE accounts.profiles SET avatar_asset_id=:asset,version=version+1,updated_at=now() WHERE user_id=:user"), {"asset": asset_id, "user": user_id})
        if old["id"]:
            retained = await connection.scalar(text("SELECT 1 FROM trust_safety.profile_reports WHERE avatar_asset_id=:id AND status IN ('pending','reviewed')"), {"id": old["id"]})
            if retained is None:
                await connection.execute(text("UPDATE media.assets SET state='deleted',updated_at=now() WHERE id=:id"), {"id": old["id"]})
...
async def delete_avatar(request: Request, token: Annotated[str | None, Cookie(alias=SESSION_COOKIE)] = None, csrf: Annotated[str | None, Header(alias=CSRF_HEADER)] = None) -> OwnProfileResponse:
    settings, _, engine = dependencies(request)
    validate_origin(request, settings)
    user_id = await mutation_user(request, token, csrf)
    old_path: Path | None = None
    async with engine.begin() as connection:
        old = (await connection.execute(text("""SELECT a.id,a.storage_key FROM accounts.profiles p LEFT JOIN media.assets a ON a.id=p.avatar_asset_id WHERE p.user_id=:user FOR UPDATE OF p"""), {"user": user_id})).mappings().one()
        await connection.execute(text("UPDATE accounts.profiles SET avatar_asset_id=NULL,version=version+1,updated_at=now() WHERE user_id=:user"), {"user": user_id})
        if old["id"]:
            retained = await connection.scalar(text("SELECT 1 FROM trust_safety.profile_reports WHERE avatar_asset_id=:id AND status IN ('pending','reviewed')"), {"id": old["id"]})
            if retained is None:
                await connection.execute(text("UPDATE media.assets SET state='deleted',updated_at=now() WHERE id=:id"), {"id": old["id"]})
...
async def avatar(public_id: str, request: Request, token: Annotated[str | None, Cookie(alias=SESSION_COOKIE)] = None) -> Response:
    await current_user(request, token)
    settings, _, engine = dependencies(request)
    async with engine.connect() as connection:
        key = await connection.scalar(text("""SELECT a.storage_key FROM accounts.profiles p JOIN accounts.users u ON u.id=p.user_id AND u.status='active' JOIN media.assets a ON a.id=p.avatar_asset_id AND a.state='ready' WHERE p.public_id=:id"""), {"id": public_id})
    if key is None:
        raise HTTPException(status_code=404, detail="avatar_not_found")
    path = settings.media_root / key
    if not path.is_file():
